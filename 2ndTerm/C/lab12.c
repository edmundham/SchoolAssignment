...
int compareScoreAscNameDescIdAsc(const record *pp, const record *qq) {
...
/*
    ascending order of scores > descending order of names > ascending order of ID
*/
int list_insert_sort(record_list *list, const record *rec) {
    node *newNode;
    node **tracer;
    ...
    for (tracer = list; *tracer != 0; tracer = &(*tracer)->next) {
        if (compareScoreAscNameDescIdAsc(&(*tracer)->data, rec) >= 0) {
            break;
        }
    }
    newNode->next = *tracer;
